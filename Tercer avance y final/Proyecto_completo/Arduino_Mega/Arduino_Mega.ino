...
void loop() {
  ...
  if (distancia > 0 && distancia < 20) { // Ajustar el umbral según sea necesario
    Serial.println("ULTRASONICO Entrada");
...
void manejarRFID(MFRC522& lector, String tipoAcceso) {
    ...
    unsigned long startTime = millis(); // Tiempo inicial para evitar bloqueo
    while (!Serial.available()) {
        if (millis() - startTime > 3000) { // Tiempo de espera (3 segundos)
            Serial.println("Error: No se recibió respuesta de Python.");
            return; // Salir de la función si no hay respuesta
        }
    }

    // Leer respuesta y limpiarla
    String respuesta = Serial.readString();
    respuesta.trim(); // Quitar espacios en blanco y caracteres extras
    Serial.println("Respuesta recibida de Python: " + respuesta);

    // Validar respuesta
    if (respuesta == tipoAcceso) {
        abrirPuerta(); // Acceso autorizado
        Serial.println("Acceso autorizado: " + tipoAcceso);
    } else if (respuesta == "Acceso denegado") {
        Serial.println("Acceso denegado.");
    } else {
        Serial.println("Formato inesperado: " + respuesta);
    }

    lector.PICC_HaltA(); // Detener lectura del lector RFID
}
